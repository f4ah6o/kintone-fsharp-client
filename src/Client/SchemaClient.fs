...
type SchemaClient(internalClient: InternalClient) =
    ...
    member this.GetApiList() : Map<string, ApiInfo> =
        let response = internalClient.Call<obj, GetApiListResponseBody>(HttpMethod.Get, KintoneApi.GetApiList, None)
        response.Apis

    /// Get API schema
    member this.GetApiSchema(apiName: string) : string * string =
        let endpoint = KintoneApi.GetApiSchema.Replace("{api}", apiName)
        let response = internalClient.Call<obj, GetApiSchemaResponseBody>(HttpMethod.Get, endpoint, None)
